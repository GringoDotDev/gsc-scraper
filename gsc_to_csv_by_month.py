...
def gsc_to_csv(
    webmasters_service, site, output, creds, start_date, end_date=default_end, gz=False
):
    """
    ...
    end_date = dm.str_to_date(
        end_date
    )  # End 3 days in the past, since GSC don't show latest data.
    ...
    while (
        start_date <= end_date
    ):  # Loop through all dates until start_date is equal to end_date.
        ...
        if csv_dt is not None and dm.date_to_str(start_date) in csv_dt:
